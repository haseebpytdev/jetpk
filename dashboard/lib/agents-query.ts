...
export function parseAgentsQuery(
  ...
  return {
    ...
    commercialStatus: parseEnum(first(searchParams.commercialStatus), COMMERCIAL_STATUSES, "all"),
    settlementStatus: parseEnum(first(searchParams.settlementStatus), SETTLEMENT_STATUSES, "all"),
    agentType: parseEnum(first(searchParams.agentType), AGENT_TYPES, "all"),
    city: first(searchParams.city),
    countryRegion: first(searchParams.countryRegion) || first(searchParams.country),
    hasOutstandingBalance: parseYesNo(first(searchParams.hasOutstandingBalance)),
    hasPendingCommission: parseYesNo(first(searchParams.hasPendingCommission)),
    ...
    sort: SORT_FIELDS.includes(sortRaw as AgentSortField) ? (sortRaw as AgentSortField) : "agentName",
...
export function agentsQueryToSearchParams(
  query: AgentsQuery,
  ...
  if (merged.commercialStatus !== "all") params.set("commercialStatus", merged.commercialStatus);
  if (merged.settlementStatus !== "all") params.set("settlementStatus", merged.settlementStatus);
  if (merged.agentType !== "all") params.set("agentType", merged.agentType);
  if (merged.city) params.set("city", merged.city);
  if (merged.countryRegion) params.set("countryRegion", merged.countryRegion);
  if (merged.hasOutstandingBalance !== "all")
    params.set("hasOutstandingBalance", merged.hasOutstandingBalance);
  if (merged.hasPendingCommission !== "all")
    params.set("hasPendingCommission", merged.hasPendingCommission);
  ...
  if (merged.sort !== "agentName") params.set("sort", merged.sort);
...
export function defaultAgentsQuery(): AgentsQuery {
  return parseAgentsQuery({});
}
