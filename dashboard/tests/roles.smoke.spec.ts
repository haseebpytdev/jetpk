...
test.beforeAll(async ({ request }) => {
  await request.get("/testdash/users/roles", { timeout: 120_000 });
});
...
test("invalid URL values fall back safely", async ({ page }) => {
  await page.goto("/testdash/users/roles?status=invalid&sort=notreal&page=-1", { waitUntil: "load" });
  await expectRolesReady(page);
  await expect(page.getByTestId("roles-metric-grid").getByRole("heading", { name: "14" })).toBeVisible();
});

test("reset filters works", async ({ page }) => {
  await page.goto("/testdash/users/roles?category=audit", { waitUntil: "load" });
...
test("pagination works", async ({ page }) => {
  await page.goto("/testdash/users/roles?pageSize=10", { waitUntil: "load" });
  await expectRolesReady(page);
...
test("drawer opens", async ({ page }) => {
  await page.goto(`/testdash/users/roles?selected=${ROLE_ID}`, { waitUntil: "load" });
...
test("protected role is marked in drawer", async ({ page }) => {
  await page.goto(`/testdash/users/roles?selected=${PROTECTED_ROLE}`, { waitUntil: "load" });
...
test("apply-to-preview adds permission chip", async ({ page }) => {
  await page.goto(`/testdash/users/roles?selected=JP-ROL-0003`, { waitUntil: "load" });
...
test("loading state works", async ({ page }) => {
  await page.goto("/testdash/users/roles?previewLoading=1", { waitUntil: "load" });
  await expect(page.getByTestId("roles-loading-state")).toBeVisible();
});

test("empty state works", async ({ page }) => {
  await page.goto("/testdash/users/roles?previewEmpty=1", { waitUntil: "load" });
  await expect(page.getByText(/No roles match your filters/i)).toBeVisible();
});

test("error state works", async ({ page }) => {
  await page.goto("/testdash/users/roles?previewError=1", { waitUntil: "load" });
