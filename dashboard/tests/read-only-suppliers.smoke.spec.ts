...
test.beforeAll(async ({ request }) => {
  expect((await request.get("/testdash/suppliers", { timeout: 120_000 })).ok()).toBeTruthy();
});
...
test("suppliers channel filter URL sync", async ({ page }) => {
  await page.goto("/testdash/suppliers?integrationStatus=Connected&sort=bookingCount", { waitUntil: "load" });
  await expect(page).toHaveURL(/integrationStatus=Connected/);
});

test("suppliers pagination URL sync", async ({ page }) => {
  await page.goto("/testdash/suppliers?page=2", { waitUntil: "load" });
...
test("suppliers forbidden preview", async ({ page }) => {
  await page.goto("/testdash/suppliers?dataSourcePreview=forbidden", { waitUntil: "load" });
  await expect(page.getByText(/Access denied/i)).toBeVisible();
});

test("suppliers unauthorized preview", async ({ page }) => {
  await page.goto("/testdash/suppliers?dataSourcePreview=unauthorized", { waitUntil: "load" });
  await expect(page.getByText(/Sign in required/i)).toBeVisible();
});

test("suppliers stale preview", async ({ page }) => {
  await page.goto("/testdash/suppliers?dataSourcePreview=stale", { waitUntil: "load" });
  await expect(page.getByTestId("stale-data-notice")).toBeVisible();
});

test("suppliers unavailable preview", async ({ page }) => {
  await page.goto("/testdash/suppliers?dataSourcePreview=unavailable", { waitUntil: "load" });
