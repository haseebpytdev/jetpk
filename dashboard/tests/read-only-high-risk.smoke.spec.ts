...
test("booking filter URL synchronization", async ({ page }) => {
  await page.goto("/admin/dashboard/bookings?status=pending&sort=departureDate&direction=asc", { waitUntil: "load" });
  await expect(page).toHaveURL(/status=pending/);
  await expect(page).toHaveURL(/sort=departureDate/);
});

test("payment filter URL synchronization", async ({ page }) => {
  await page.goto("/admin/dashboard/payments?paymentStatus=paid&sort=grossAmount", { waitUntil: "load" });
  await expect(page).toHaveURL(/paymentStatus=paid/);
});

test("customer filter URL synchronization", async ({ page }) => {
  await page.goto("/admin/dashboard/customers?verificationStatus=Verified&sort=bookingCount", { waitUntil: "load" });
...
test("stale data notice via preview gate", async ({ page }) => {
  await page.goto("/admin/dashboard/bookings?dataSourcePreview=stale", { waitUntil: "load" });
  await expect(page.getByTestId("stale-data-notice")).toBeVisible();
});

test("unavailable to error transition gates", async ({ page }) => {
  await page.goto("/admin/dashboard/customers?dataSourcePreview=unavailable", { waitUntil: "load" });
  await expect(page.getByText(/Service unavailable/i)).toBeVisible();
  await page.goto("/admin/dashboard/customers?dataSourcePreview=error", { waitUntil: "load" });
...
test("pagination on customers", async ({ page }) => {
  await page.goto("/admin/dashboard/customers?page=2&pageSize=10", { waitUntil: "load" });
  await expect(page).toHaveURL(/page=2/);
  await expect(page).toHaveURL(/pageSize=10/);
});

test("supplier channel filter URL sync", async ({ page }) => {
  await page.goto("/admin/dashboard/suppliers?integrationStatus=Connected", { waitUntil: "load" });
  await expect(page).toHaveURL(/integrationStatus=Connected/);
});

test("agent filter URL sync", async ({ page }) => {
  await page.goto("/admin/dashboard/agents?accountStatus=Active", { waitUntil: "load" });
  await expect(page).toHaveURL(/accountStatus=Active/);
});

test("pnr order type filter URL", async ({ page }) => {
  await page.goto("/admin/dashboard/pnrs?referenceType=NDC+Order", { waitUntil: "load" });
  await expect(page).toHaveURL(/referenceType=/);
});

test("ticket filter URL sync", async ({ page }) => {
  await page.goto("/admin/dashboard/tickets?issueStatus=Issued", { waitUntil: "load" });
  await expect(page).toHaveURL(/issueStatus=Issued/);
});

test("report date range URL", async ({ page }) => {
  await page.goto("/admin/dashboard/reports?datePreset=last_30_days", { waitUntil: "load" });
  await expect(page).toHaveURL(/datePreset=last_30_days/);
});

test("report currency filter", async ({ page }) => {
  await page.goto("/admin/dashboard/reports?currency=PKR", { waitUntil: "load" });
...
test("GDS NDC distinction in fixture pnrs", async ({ page }) => {
  await page.goto("/admin/dashboard/pnrs?referenceType=GDS+PNR&pageSize=50", { waitUntil: "load" });
  await expect(page.getByText("GDS PNR").first()).toBeVisible({ timeout: 30_000 });
  await page.goto("/admin/dashboard/pnrs?referenceType=NDC+Order&pageSize=50", { waitUntil: "load" });
