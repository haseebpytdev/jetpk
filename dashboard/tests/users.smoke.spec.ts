...
test.beforeAll(async ({ request }) => {
  await request.get("/testdash/users", { timeout: 120_000 });
});
...
test("invalid URL values fall back safely", async ({ page }) => {
  await page.goto("/testdash/users?status=invalid&sort=notreal&page=-1", { waitUntil: "load" });
  await expectUsersReady(page);
  await expect(page.getByTestId("users-metric-grid").getByRole("heading", { name: "40" })).toBeVisible();
});

test("reset filters works", async ({ page }) => {
  await page.goto("/testdash/users?status=locked", { waitUntil: "load" });
...
test("pagination works", async ({ page }) => {
  await page.goto("/testdash/users?pageSize=10", { waitUntil: "load" });
  await expectUsersReady(page);
...
test("drawer opens", async ({ page }) => {
  await page.goto(`/testdash/users?selected=${USER_ID}`, { waitUntil: "load" });
...
test("role preview is labelled preview-only", async ({ page }) => {
  await page.goto(`/testdash/users?selected=${NO_ROLE_USER}`, { waitUntil: "load" });
...
test("empty state works", async ({ page }) => {
  await page.goto("/testdash/users?previewEmpty=1", { waitUntil: "load" });
  await expect(page.getByText(/No users match your filters/i)).toBeVisible();
});

test("error state works", async ({ page }) => {
  await page.goto("/testdash/users?previewError=1", { waitUntil: "load" });
