...
test.beforeAll(async ({ request }) => {
  expect((await request.get("/admin/dashboard/agents", { timeout: 120_000 })).ok()).toBeTruthy();
});
...
test("fixture source notice on agents", async ({ page }) => {
  await page.goto("/admin/dashboard/agents?dataSourcePreview=fixture", { waitUntil: "load" });
...
test("agent filter URL sync", async ({ page }) => {
  await page.goto("/admin/dashboard/agents?accountStatus=Active&sort=bookingCount", { waitUntil: "load" });
...
test("agents forbidden preview", async ({ page }) => {
  await page.goto("/admin/dashboard/agents?dataSourcePreview=forbidden", { waitUntil: "load" });
...
test("agents stale preview", async ({ page }) => {
  await page.goto("/admin/dashboard/agents?dataSourcePreview=stale", { waitUntil: "load" });
  await expect(page.getByTestId("stale-data-notice")).toBeVisible();
});

test("agents error preview", async ({ page }) => {
  await page.goto("/admin/dashboard/agents?dataSourcePreview=error", { waitUntil: "load" });
...
test("agents pagination URL sync", async ({ page }) => {
  await page.goto("/admin/dashboard/agents?page=2&pageSize=10", { waitUntil: "load" });
