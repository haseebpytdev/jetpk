...
const routes = [
  { path: "/admin/dashboard/reports", label: "Overview" },
  { path: "/admin/dashboard/reports/sales", label: "Sales" },
  { path: "/admin/dashboard/reports/bookings", label: "Bookings" },
  { path: "/admin/dashboard/reports/payments", label: "Payments" },
  { path: "/admin/dashboard/reports/operations", label: "Operations" },
];
...
test("reports navigation between sections", async ({ page }) => {
  ...
  await navigateReportsSection(page, "Operations", /\/admin\/dashboard\/reports\/operations/);
});
...
test("custom date range shows validation error", async ({ page }) => {
  await page.goto("/admin/dashboard/reports?datePreset=custom&startDate=2026-08-01&endDate=2026-07-01", { waitUntil: "load" });
...
test("invalid URL state falls back safely", async ({ page }) => {
  await page.goto("/admin/dashboard/reports?datePreset=invalid&currency=XYZ", { waitUntil: "load" });
  await expect(page.getByRole("heading", { name: "Reports", level: 1 })).toBeVisible();
});

test("reset filters works", async ({ page }) => {
  await page.goto("/admin/dashboard/reports?supplier=Sabre&channel=agent", { waitUntil: "load" });
...
test("browser back forward preserves navigation", async ({ page }) => {
  ...
  await expect(page).toHaveURL(/\/admin\/dashboard\/reports\/sales/);
  await expectReportsReady(page);
});
...
test("export controls are accessible", async ({ page }) => {
  await page.goto("/admin/dashboard/reports/sales", { waitUntil: "load" });
