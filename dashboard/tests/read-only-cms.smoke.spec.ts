...
test.beforeAll(async ({ request }) => {
  expect((await request.get("/testdash/cms", { timeout: 120_000 })).ok()).toBeTruthy();
});
...
test("fixture source notice on CMS", async ({ page }) => {
  await page.goto("/testdash/cms?dataSourcePreview=fixture", { waitUntil: "load" });
...
test("CMS filter URL sync", async ({ page }) => {
  await page.goto("/testdash/cms/pages?status=published", { waitUntil: "load" });
...
test("CMS forbidden preview state", async ({ page }) => {
  await page.goto("/testdash/cms?dataSourcePreview=forbidden", { waitUntil: "load" });
...
test("CMS live read-only notice", async ({ page }) => {
  await page.goto("/testdash/cms?dataSourcePreview=live", { waitUntil: "load" });
