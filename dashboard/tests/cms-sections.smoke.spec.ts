...
test.beforeAll(async ({ request }) => {
  await request.get("/testdash/cms/sections", { timeout: 120_000 });
});

test("sections table renders", async ({ page }) => {
  await page.goto("/testdash/cms/sections", { waitUntil: "load" });
...
test("component keys visible in table", async ({ page }) => {
  await page.goto("/testdash/cms/sections?sectionType=homepage.hero", { waitUntil: "load" });
...
test("section drawer opens", async ({ page }) => {
  await page.goto(`/testdash/cms/sections?selected=${SECTION_ID}`, { waitUntil: "load" });
...
test("flight-search logic is not editable", async ({ page }) => {
  ...
  await page.goto(`/testdash/cms/sections?selected=${flightSection}`, { waitUntil: "load" });
...
test("more than three offers require carousel", async ({ page }) => {
  ...
  await page.goto(`/testdash/cms/sections?selected=${offersSection}`, { waitUntil: "load" });
