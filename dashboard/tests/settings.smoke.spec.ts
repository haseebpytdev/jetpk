...
test.beforeAll(async ({ request }) => {
  await request.get("/testdash/settings", { timeout: 120_000 });
});
...
test("notification settings route renders", async ({ page }) => {
  await page.goto("/testdash/settings/notifications", { waitUntil: "load" });
...
test("browser back and forward works", async ({ page }) => {
  await page.goto("/testdash/settings/integrations", { waitUntil: "load" });
  await expectSettingsReady(page);
  await page.goto("/testdash/settings/integrations?validationState=blocked", { waitUntil: "load" });
...
test("overview open link navigates to general", async ({ page }) => {
  ...
  await expect(page).toHaveURL(/\/testdash\/settings\/general/, { timeout: 30_000 });
  await expectSettingsReady(page);
});
...
test("loading state works", async ({ page }) => {
  await page.goto("/testdash/settings?previewLoading=1", { waitUntil: "load" });
...
test("error state works", async ({ page }) => {
  await page.goto("/testdash/settings?previewError=1", { waitUntil: "load" });
  await expect(page.getByText(/Unable to load settings/i)).toBeVisible();
});

test("invalid URL values fall back safely", async ({ page }) => {
  await page.goto("/testdash/settings/general?validationState=invalid", { waitUntil: "load" });
  await expectSettingsReady(page);
  await expect(page.getByTestId("general-settings-workspace")).toBeVisible();
});
...
test("all settings subroutes render preview banner", async ({ page }) => {
  const routes = [
    "/testdash/settings",
    "/testdash/settings/general",
    "/testdash/settings/security",
    "/testdash/settings/notifications",
    "/testdash/settings/integrations",
  ];
