...
test.beforeAll(async ({ request }) => {
  expect((await request.get("/admin/dashboard/users/permissions", { timeout: 120_000 })).ok()).toBeTruthy();
});
...
test("permissions source notice", async ({ page }) => {
  await page.goto("/admin/dashboard/users/permissions?dataSourcePreview=fixture", { waitUntil: "load" });
...
test("permissions drawer deep link", async ({ page }) => {
  await page.setViewportSize({ width: 1280, height: 720 });
  await page.goto("/admin/dashboard/users/permissions?selected=JP-PRM-0001", { waitUntil: "load" });
