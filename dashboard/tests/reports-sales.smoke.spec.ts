...
test.beforeAll(async ({ request }) => {
  expect((await request.get("/admin/dashboard/reports/sales", { timeout: 120_000 })).ok()).toBeTruthy();
});
...
test("sales export reflects current filters", async ({ page }) => {
  await page.goto("/admin/dashboard/reports/sales?datePreset=last_7_days", { waitUntil: "load" });
