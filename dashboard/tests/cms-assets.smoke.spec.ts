...
test.beforeAll(async ({ request }) => {
  await request.get("/testdash/cms/assets", { timeout: 120_000 });
});

test("asset table renders", async ({ page }) => {
  await page.goto("/testdash/cms/assets", { waitUntil: "load" });
...
test("asset drawer opens", async ({ page }) => {
  await page.goto(`/testdash/cms/assets?selected=${ASSET_ID}`, { waitUntil: "load" });
...
test("alt-text state displays", async ({ page }) => {
  await page.goto(`/testdash/cms/assets?selected=${MISSING_ALT_ASSET}`, { waitUntil: "load" });
  const drawer = page.getByTestId("cms-asset-drawer");
...
test("loading state renders", async ({ page }) => {
  await page.goto("/testdash/cms/assets?previewLoading=1", { waitUntil: "load" });
  await expect(page.getByTestId("cms-loading-state")).toBeVisible();
});

test("empty state renders", async ({ page }) => {
  await page.goto("/testdash/cms/assets?previewEmpty=1", { waitUntil: "load" });
...
test("pagination works", async ({ page }) => {
  await page.goto("/testdash/cms/assets?page=2", { waitUntil: "load" });
