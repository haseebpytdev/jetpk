...
test.beforeAll(async ({ request }) => {
  await request.get("/admin/dashboard/users/permissions", { timeout: 120_000 });
});

test("permissions route renders", async ({ page }) => {
  await page.goto("/admin/dashboard/users/permissions", { waitUntil: "load" });
...
test("invalid URL values fall back safely", async ({ page }) => {
  await page.goto("/admin/dashboard/users/permissions?domain=invalid&sort=notreal&page=-1", { waitUntil: "load" });
  await expectPermissionsReady(page);
...
test("reset filters works", async ({ page }) => {
  await page.goto("/admin/dashboard/users/permissions?domain=bookings", { waitUntil: "load" });
...
test("pagination works", async ({ page }) => {
  await page.goto("/admin/dashboard/users/permissions?pageSize=10", { waitUntil: "load" });
  await expectPermissionsReady(page);
...
test("drawer opens", async ({ page }) => {
  await page.goto(`/admin/dashboard/users/permissions?selected=${PERMISSION_ID}`, { waitUntil: "load" });
...
test("high-risk permission is marked in drawer", async ({ page }) => {
  await page.goto("/admin/dashboard/users/permissions?selected=JP-PRM-0006", { waitUntil: "load" });
...
test("loading state works", async ({ page }) => {
  await page.goto("/admin/dashboard/users/permissions?previewLoading=1", { waitUntil: "load" });
  await expect(page.getByTestId("permissions-loading-state")).toBeVisible();
});

test("empty state works", async ({ page }) => {
  await page.goto("/admin/dashboard/users/permissions?previewEmpty=1", { waitUntil: "load" });
  await expect(page.getByText(/No permissions match your filters/i)).toBeVisible();
});

test("error state works", async ({ page }) => {
  await page.goto("/admin/dashboard/users/permissions?previewError=1", { waitUntil: "load" });
