...
test.beforeAll(async ({ request }) => {
  await request.get("/admin/dashboard/cms/banners", { timeout: 120_000 });
});

test("banner table renders", async ({ page }) => {
  await page.goto("/admin/dashboard/cms/banners", { waitUntil: "load" });
...
test("banner drawer opens", async ({ page }) => {
  await page.goto(`/admin/dashboard/cms/banners?selected=${BANNER_ID}`, { waitUntil: "load" });
...
test("invalid placement warning displays for offer banners", async ({ page }) => {
  ...
  await page.goto(`/admin/dashboard/cms/banners?selected=${offerBanner}`, { waitUntil: "load" });
  const drawer = page.getByTestId("cms-banner-drawer");
...
test("pagination works", async ({ page }) => {
  await page.goto("/admin/dashboard/cms/banners?page=2", { waitUntil: "load" });
...
test("loading state renders", async ({ page }) => {
  await page.goto("/admin/dashboard/cms/banners?previewLoading=1", { waitUntil: "load" });
  await expect(page.getByTestId("cms-loading-state")).toBeVisible();
});

test("empty state renders", async ({ page }) => {
  await page.goto("/admin/dashboard/cms/banners?previewEmpty=1", { waitUntil: "load" });
