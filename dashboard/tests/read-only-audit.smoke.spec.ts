...
test.beforeAll(async ({ request }) => {
  expect((await request.get("/admin/dashboard/audit", { timeout: 120_000 })).ok()).toBeTruthy();
});
...
test("audit source notice", async ({ page }) => {
  await page.goto("/admin/dashboard/audit?dataSourcePreview=fixture", { waitUntil: "load" });
  await expect(page.getByTestId("fixture-data-notice").first()).toBeVisible();
});

test("audit filters URL sync", async ({ page }) => {
  await page.goto("/admin/dashboard/audit?category=security", { waitUntil: "load" });
...
test("audit drawer deep link", async ({ page }) => {
  await page.setViewportSize({ width: 1280, height: 720 });
  await page.goto("/admin/dashboard/audit?selected=JP-AUD-0001", { waitUntil: "load" });
