...
test.beforeAll(async ({ request }) => {
  const response = await request.get("/admin/dashboard/cms", { timeout: 120_000 });
...
test("attention queue links correctly", async ({ page }) => {
  ...
  await expect(firstLink).toHaveAttribute("href", /\/admin\/dashboard\/cms\//);
});
...
test("invalid URL state falls back on overview", async ({ page }) => {
  await page.goto("/admin/dashboard/cms?status=bad&previewMode=invalid", { waitUntil: "load" });
  await expectCmsReady(page);
});
...
test("browser back from pages to overview", async ({ page }) => {
  ...
  await navigateCmsSection(page, "Pages", /\/admin\/dashboard\/cms\/pages/);
  await page.goBack();
  await expect(page).toHaveURL(/\/admin\/dashboard\/cms\/?$/, { timeout: 30_000 });
  await expectCmsReady(page);
});
