...
test.beforeAll(async ({ request }) => {
  await request.get("/admin/dashboard/cms/notices", { timeout: 120_000 });
});

test("notice table renders", async ({ page }) => {
  await page.goto("/admin/dashboard/cms/notices", { waitUntil: "load" });
...
test("notice drawer opens", async ({ page }) => {
  await page.goto(`/admin/dashboard/cms/notices?selected=${NOTICE_ID}`, { waitUntil: "load" });
...
test("reset filters works", async ({ page }) => {
  await page.goto("/admin/dashboard/cms/notices?noticeSeverity=warning", { waitUntil: "load" });
...
test("loading state renders", async ({ page }) => {
  await page.goto("/admin/dashboard/cms/notices?previewLoading=1", { waitUntil: "load" });
  await expect(page.getByTestId("cms-loading-state")).toBeVisible();
});

test("empty state renders", async ({ page }) => {
  await page.goto("/admin/dashboard/cms/notices?previewEmpty=1", { waitUntil: "load" });
  await expect(page.getByText("No CMS records match filters")).toBeVisible();
});

test("pagination works", async ({ page }) => {
  await page.goto("/admin/dashboard/cms/notices?page=2", { waitUntil: "load" });
