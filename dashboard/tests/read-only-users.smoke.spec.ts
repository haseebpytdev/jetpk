...
test.beforeAll(async ({ request }) => {
  expect((await request.get("/testdash/users", { timeout: 120_000 })).ok()).toBeTruthy();
});
...
test("fixture source notice on users", async ({ page }) => {
  await page.goto("/testdash/users?dataSourcePreview=fixture", { waitUntil: "load" });
...
test("users filter URL sync", async ({ page }) => {
  await page.goto("/testdash/users?status=active", { waitUntil: "load" });
  await expect(page).toHaveURL(/status=active/);
});

test("users drawer deep link", async ({ page }) => {
  await page.setViewportSize({ width: 1280, height: 720 });
  await page.goto("/testdash/users?selected=JP-USR-0001", { waitUntil: "load" });
...
test("users forbidden preview", async ({ page }) => {
  await page.goto("/testdash/users?dataSourcePreview=forbidden", { waitUntil: "load" });
