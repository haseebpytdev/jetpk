...
test.beforeAll(async ({ request }) => {
  expect((await request.get("/admin/dashboard/tickets", { timeout: 120_000 })).ok()).toBeTruthy();
});
...
test("fixture source notice on tickets", async ({ page }) => {
  await page.goto("/admin/dashboard/tickets?dataSourcePreview=fixture", { waitUntil: "load" });
...
test("ticket filter URL sync", async ({ page }) => {
  await page.goto("/admin/dashboard/tickets?issueStatus=Issued&documentType=E-Ticket", { waitUntil: "load" });
...
test("tickets forbidden preview", async ({ page }) => {
  await page.goto("/admin/dashboard/tickets?dataSourcePreview=forbidden", { waitUntil: "load" });
