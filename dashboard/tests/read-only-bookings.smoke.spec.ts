...
test.beforeAll(async ({ request }) => {
  expect((await request.get("/admin/dashboard/bookings", { timeout: 120_000 })).ok()).toBeTruthy();
});
...
test("bookings filter URL sync status", async ({ page }) => {
  await page.setViewportSize({ width: 1280, height: 720 });
  await page.goto("/admin/dashboard/bookings?status=confirmed", { waitUntil: "load" });
  await expect(page).toHaveURL(/status=confirmed/);
});

test("bookings pagination URL sync", async ({ page }) => {
  await page.goto("/admin/dashboard/bookings?page=2", { waitUntil: "load" });
...
test("bookings forbidden preview state", async ({ page }) => {
  await page.goto("/admin/dashboard/bookings?dataSourcePreview=forbidden", { waitUntil: "load" });
