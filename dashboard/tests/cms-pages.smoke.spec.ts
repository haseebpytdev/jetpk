...
test("desktop day preview renders", async ({ page }) => {
  await page.goto(`/admin/dashboard/cms/pages?selected=${PAGE_ID}&previewMode=desktop_day`, { waitUntil: "load" });
  await expect(page.getByTestId("cms-preview-frame")).toBeVisible();
});

test("desktop night preview renders", async ({ page }) => {
  await page.goto(`/admin/dashboard/cms/pages?selected=${PAGE_ID}&previewMode=desktop_night`, { waitUntil: "load" });
...
test("mobile day preview renders", async ({ page }) => {
  await page.goto(`/admin/dashboard/cms/pages?selected=${PAGE_ID}&previewMode=mobile_day`, { waitUntil: "load" });
  await expect(page.getByTestId("cms-preview-frame")).toBeVisible();
});

test("mobile night preview renders", async ({ page }) => {
  await page.goto(`/admin/dashboard/cms/pages?selected=${PAGE_ID}&previewMode=mobile_night`, { waitUntil: "load" });
  await expect(page.getByTestId("cms-preview-frame")).toBeVisible();
});
...
test("pagination updates URL", async ({ page }) => {
  await page.goto("/admin/dashboard/cms/pages?page=2", { waitUntil: "load" });
  await expect(page).toHaveURL(/page=2/);
});

test("reset filters works", async ({ page }) => {
  await page.goto("/admin/dashboard/cms/pages?pageType=homepage", { waitUntil: "load" });
...
test("loading state on pages", async ({ page }) => {
  await page.goto("/admin/dashboard/cms/pages?previewLoading=1", { waitUntil: "load" });
  await expect(page.getByTestId("cms-loading-state")).toBeVisible();
});

test("empty state on pages", async ({ page }) => {
  await page.goto("/admin/dashboard/cms/pages?previewEmpty=1", { waitUntil: "load" });
