...
test.beforeAll(async ({ request }) => {
  await request.get("/testdash/audit", { timeout: 120_000 });
});
...
test("summary metrics derive from fixtures", async ({ page }) => {
  await page.goto("/testdash/audit?datePreset=this_month", { waitUntil: "load" });
...
test("custom date range shows validation error", async ({ page }) => {
  await page.goto("/testdash/audit?datePreset=custom&startDate=2026-08-01&endDate=2026-07-01", { waitUntil: "load" });
...
test("invalid URL values fall back safely", async ({ page }) => {
  await page.goto("/testdash/audit?datePreset=this_month&category=invalid&sort=notreal&page=-1", { waitUntil: "load" });
  await expectAuditReady(page);
  await expect(
...
test("reset filters works", async ({ page }) => {
  await page.goto("/testdash/audit?category=authentication", { waitUntil: "load" });
...
test("pagination works", async ({ page }) => {
  await page.goto("/testdash/audit?pageSize=10", { waitUntil: "load" });
  await expectAuditReady(page);
...
test("drawer opens", async ({ page }) => {
  await page.goto(`/testdash/audit?selected=${EVENT_ID}`, { waitUntil: "load" });
...
test("target link renders in drawer", async ({ page }) => {
  await page.goto(`/testdash/audit?selected=${ACTOR_USER_EVENT}`, { waitUntil: "load" });
  const drawer = page.getByTestId("audit-event-detail-drawer");
