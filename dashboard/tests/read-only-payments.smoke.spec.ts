...
test.beforeAll(async ({ request }) => {
  expect((await request.get("/testdash/payments", { timeout: 120_000 })).ok()).toBeTruthy();
});
...
test("payments fixture notice", async ({ page }) => {
  await page.goto("/testdash/payments?dataSourcePreview=fixture", { waitUntil: "load" });
...
test("payments filter URL sync reconciliation", async ({ page }) => {
  await page.goto("/testdash/payments?reconciliation=unreconciled", { waitUntil: "load" });
  await expect(page).toHaveURL(/reconciliation=unreconciled/);
});

test("payments pagination URL sync", async ({ page }) => {
  await page.goto("/testdash/payments?page=2", { waitUntil: "load" });
  await expect(page).toHaveURL(/page=2/);
});

test("payments unavailable preview state", async ({ page }) => {
  await page.goto("/testdash/payments?dataSourcePreview=unavailable", { waitUntil: "load" });
...
test("payments live read-only notice", async ({ page }) => {
  await page.goto("/testdash/payments?dataSourcePreview=live", { waitUntil: "load" });
