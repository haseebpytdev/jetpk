...
test.beforeAll(async ({ request }) => {
  expect((await request.get("/testdash/customers", { timeout: 120_000 })).ok()).toBeTruthy();
});
...
test("customers fixture notice", async ({ page }) => {
  await page.goto("/testdash/customers?dataSourcePreview=fixture", { waitUntil: "load" });
...
test("customers filter URL sync account status", async ({ page }) => {
  await page.goto("/testdash/customers?accountStatus=Active", { waitUntil: "load" });
  await expect(page).toHaveURL(/accountStatus=Active/);
});

test("customers pagination URL sync", async ({ page }) => {
  await page.goto("/testdash/customers?page=2", { waitUntil: "load" });
  await expect(page).toHaveURL(/page=2/);
});

test("customers drawer with selected id", async ({ page }) => {
  await page.setViewportSize({ width: 1280, height: 720 });
  await page.goto("/testdash/customers?id=JP-CU-40026", { waitUntil: "load" });
  await expect(page.getByRole("dialog")).toBeVisible({ timeout: 30_000 });
});

test("customers forbidden preview state", async ({ page }) => {
  await page.goto("/testdash/customers?dataSourcePreview=forbidden", { waitUntil: "load" });
...
test("customers live read-only notice", async ({ page }) => {
  await page.goto("/testdash/customers?dataSourcePreview=live", { waitUntil: "load" });
