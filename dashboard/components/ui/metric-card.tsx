...
export function MetricCard({
  ...
  return (
    <div
      className={cn(
        "min-w-0 rounded-2xl border border-jp-border bg-jp-card px-4 py-3 shadow-sm",
        className,
      )}
    >
      <p className="text-xs font-medium uppercase tracking-wide text-jp-muted">{label}</p>
      <p className="mt-1 truncate text-xl font-semibold tabular-nums text-gray-900">{value}</p>
