<<<<<<< HEAD
# Asif Travels OTA (Laravel)

Laravel **database-backed** OTA for **Asif Travels**: public flight search and checkout, **Duffel**-driven offers when a connection is configured, authenticated **admin / staff / agent / and customer** areas, booking operations, payments and refunds, documents, reports, supplier diagnostics, and a **notification and scheduled reporting** layer (email, agency-scoped settings, and console-scheduled jobs where enabled).
=======
...
<<<<<<< HEAD
- **Admin:** dashboard, **booking management** (list, filters, detail “command center” with tabs), payments, refunds, cancellations, ticketing actions, documents, communications, **API / supplier settings**, agents, applications, commissions, **reports**, **supplier diagnostics**, branding/homepage/media, and safety/checklist-style screens.
=======
...
<<<<<<< HEAD
- **WhatsApp / channels:** settings may be present; “ready” does not mean full outbound productization.
=======
...
<<<<<<< HEAD
- **End-to-end certification** of every path for production traffic (load, observability, runbooks) — not claimed as finished in this README.
=======
...
<<<<<<< HEAD
| **Duffel integration** | Primary integrated path for offers/booking when connection and token are set (Admin → API settings / `SupplierConnection`). |
=======
...
<<<<<<< HEAD
- Run the app: `php artisan serve --host=127.0.0.1 --port=8000` — then open **http://127.0.0.1:8000**
=======
...
<<<<<<< HEAD
- The **server environment uses its own `.env`** — distinct from any developer machine. **Do not commit** `.env` or real credentials.
=======
