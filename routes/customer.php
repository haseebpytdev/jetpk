...
Route::prefix('customer')->name('customer.')->group(function (): void {
    Route::middleware('platform.module:customer_portal')->group(function (): void {
        ...
        Route::get('/notifications/unread-summary', [CustomerNotificationController::class, 'unreadSummary'])->name('notifications.unread-summary');
    ...
    Route::post('/bookings/{booking:booking_reference}/payment-proof', [CustomerBookingController::class, 'submitPaymentProof'])
        ->middleware(['platform.module:payment_proofs', 'throttle:payment-proof-submit'])
        ->name('bookings.payment-proof');
    Route::post('/bookings/{booking:booking_reference}/promo/apply', [BookingCheckoutPromoController::class, 'apply'])
        ->middleware('throttle:promo-apply')
        ->name('bookings.promo.apply');
    Route::post('/bookings/{booking:booking_reference}/promo/remove', [BookingCheckoutPromoController::class, 'remove'])
        ->middleware('throttle:promo-apply')
    ...
    Route::middleware('platform.module:support_system')->group(function (): void {
        ...
        Route::patch('/support/tickets/{ticket:ticket_reference}/close', [SupportTicketController::class, 'close'])->name('support.tickets.close');
    });
});
