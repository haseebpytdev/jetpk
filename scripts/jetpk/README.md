...
## Post-recovery deploy guards (mandatory)

Fail-closed helpers used before build/restart:

```bash
bash scripts/jetpk/guard-dirty-worktree.sh
DISK_PATH=/home/pkjetp MIN_FREE_GB=8 bash scripts/jetpk/guard-disk-space.sh
AUTHORIZED_SHA=<sha> ALLOWLIST_SHAS="<active> <rollback>" \
  bash scripts/jetpk/guard-authorized-sha.sh
APP_DIR=/home/pkjetp/jetpk_app/frontend EXPECTED_BUILD_ID=<id> \
  REQUIRE_SOURCE_SHA=<sha> bash scripts/jetpk/guard-build-before-restart.sh
ACTIVE_SHA=<active> ROLLBACK_SHA=<rollback> \
  bash scripts/jetpk/release-retention-dry-run.sh   # default DRY-RUN
```

Composite:

```bash
AUTHORIZED_SHA=<sha> APP_DIR=/home/pkjetp/jetpk_app/frontend \
...
```bash
bash scripts/jetpk/test-post-recovery-guards.sh
```

Gates emit `WORKTREE_DIRTY`, `SOURCE_SHA_NOT_AUTHORIZED`, `BUILD_SOURCE_SHA_MISMATCH`,
`BUILD_BEFORE_RESTART_GUARD`, `DISK_SPACE_GUARD`, `RELEASE_RETENTION_GUARD`.
Never stop/restart public or dashboard Next until a valid replacement `.next` exists.
