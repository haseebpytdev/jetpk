...
/**
 * BF7-D controlled CERT Brand object variant matrix (one POST per invocation).
 *
 * Usage:
 *   php scripts/bf7d-controlled-cert-brand-variant.php --booking=51 --variant=object_value [--skip-send]
 *   php scripts/bf7d-controlled-cert-brand-variant.php --booking 51 --variant object_value --allow-production-cert-controlled-send
 *   php scripts/bf7d-controlled-cert-brand-variant.php --self-test-cli-parse
 */
 ...
 * }
 */
function parseBf7dArgv(array $argv): array
{
    $bookingId = null;
    $variant = null;
    $skipSend = false;
    ...
    for ($i = 0; $i < count($args); $i++) {
        ...
        if ($arg === '--self-test-cli-parse') {
            ...
            $i++;
        } elseif (str_starts_with($arg, '--variant=')) {
            $variant = strtolower(trim(substr($arg, 10)));
        } elseif ($arg === '--variant' && isset($args[$i + 1]) && trim($args[$i + 1]) !== '') {
            $variant = strtolower(trim($args[$i + 1]));
            $i++;
        }
    }
...
function runBf7dCliParseSelfTest(): int
{
    $failures = [];
    ...
    $eq('variant_equals_form', 'object_value', $parsed['variant']);

    $parsed = parseBf7dArgv(['script', '--booking', '51', '--variant', 'object_value']);
    $eq('booking_space_form', 51, $parsed['booking_id']);
    $eq('variant_space_form', 'object_value', $parsed['variant']);

    $parsed = parseBf7dArgv(['script', '--booking=51', '--variant=object_value', '--allow-production-cert-controlled-send', '--skip-send']);
    ...
    $eq('gate_production_blocked', 'APP_ENV must be local or testing, or pass --allow-production-cert-controlled-send on production.', resolveAppEnvGate(false, 'production'));
...
function setBf7dCompareTestFlags(string $variant): void
{
...
function setBf7dTemporaryBookingSendFlags(): void
{
...
function flagsRestored(array $before, array $after): bool
{
 ...
 */
function brandPointerErrorPresent(array $paths, array $messages): bool
{
 ...
 * @param  list<string>  $messages
 */
function brandCodePropertyErrorPresent(array $paths, array $messages): bool
{
 ...
 * @param  list<string>  $messages
 */
function stringTypeErrorPresent(array $paths, array $messages): bool
{
    foreach ($messages as $message) {
        $msg = strtolower((string) $message);
        if (str_contains($msg, 'instance type (string)') && str_contains($msg, 'allowed: ["object"]')) {
            return true;
        }
    }
...
{
    return brandCodePropertyErrorPresent($paths, $messages) && brandPointerErrorPresent($paths, $messages);
}
...
function classifySabreRow(array $row): string
{
    ...
    if ($httpStatus === '400') {
        if (stringTypeErrorPresent($paths, $messages)) {
            return 'brand_string_type_rejected_400';
        }
        if (brandCodePropertyErrorPresent($paths, $messages)) {
            return 'brand_object_properties_rejected_400';
        }
...
function emitPreflightToStderr(array $preflight): void
{
    $lines = [
        ...
        'variant='.($preflight['variant'] ?? 'null'),
...
$cli = parseBf7dArgv($argv ?? []);
if ($cli['self_test_cli_parse']) {
    exit(runBf7dCliParseSelfTest());
}

$bf7dDirectInvocation = PHP_SAPI === 'cli'
    && isset($argv[0])
    && realpath($argv[0]) === realpath(__FILE__);

if (! $bf7dDirectInvocation) {
    return;
}
...
const BF7D_ENDPOINT_PATH = SabreBookingPayloadBuilder::PASSENGER_RECORDS_V24_CREATE_PATH;
const BF7D_PAYLOAD_STYLE = SabreBookingPayloadBuilder::IATI_LIKE_CPNR_V2_4_GDS;
const BF7D_BLOCKED_BOOKING_IDS = [43, 46];

/** @var array<string, mixed>|null */
$bf7dOriginalFlagSnapshot = null;

$appEnv = (string) config('app.env');
$allowProduction = $cli['allow_production_cert_controlled_send'];
$skipSend = $cli['skip_send'];
$bookingOpt = $cli['booking_id'];
$variantOpt = $cli['variant'];

$bf7dOriginalFlagSnapshot = flagSnapshot();

$report = [
    'sprint' => 'SABRE-BRANDED-FARES-BF7-D',
    'app_env' => $appEnv,
    'allow_production_cert_controlled_send' => $allowProduction,
    'flags_before' => $bf7dOriginalFlagSnapshot,
    ...
    'brand_code' => null,
    'variant' => $variantOpt,
    'endpoint_path' => BF7D_ENDPOINT_PATH,
    'endpoint_host' => null,
    'payload_style' => BF7D_PAYLOAD_STYLE,
    ...
    'string_type_error_present' => null,
    'pnr_created' => false,
    'pnr' => null,
    'status' => 'pending',
];
...
try {
    ...
    if ($variantOpt === null || $variantOpt === '') {
        throw new RuntimeException('Missing --variant={name} (e.g. --variant=object_value).');
    }

    $builder = app(SabreBookingPayloadBuilder::class);
    if (! $builder->isSupportedAirPriceBrandShapeCompareVariant($variantOpt)) {
        $report['status'] = 'blocked_precheck';
        $report['sabre_classification'] = 'unknown_variant';
        throw new RuntimeException('Unknown variant "'.$variantOpt.'". Supported: '.implode(', ', SabreBookingPayloadBuilder::AIRPRICE_BRAND_SHAPE_COMPARE_VARIANTS));
    }
    ...
    if (in_array($bookingOpt, BF7D_BLOCKED_BOOKING_IDS, true)) {
        $report['status'] = 'blocked_precheck';
    ...
    if (BF7D_ENDPOINT_PATH !== SabreBookingPayloadBuilder::PASSENGER_RECORDS_V24_CREATE_PATH) {
        $report['status'] = 'blocked_precheck';
    ...
    if ($brandCode !== 'FL') {
        ...
        throw new RuntimeException('Controlled BF7-D test requires brand_code FL (got '.($brandCode ?? 'null').').');
    }
    ...
    setBf7dCompareTestFlags($variantOpt);
    $report['flags_during_test'] = flagSnapshot();

    /** @var SabreBookingService $sabreBooking */
    $sabreBooking = app(SabreBookingService::class);

    $diag = $sabreBooking->inspectPassengerRecordsAirPriceBrandDiagnosticsForCommand($booking, BF7D_PAYLOAD_STYLE);
    ...
    if ($skipSend) {
        ...
        setBf7dTemporaryBookingSendFlags();
        ...
        $messages = is_array($row['response_error_messages'] ?? null)
            ...
            : [];
        $report['sabre_classification'] = classifySabreRow(array_merge($row, [
            'response_error_paths' => $paths,
            'response_error_messages' => $messages,
        ]));
        $report['brand_pointer_error_present'] = brandPointerErrorPresent($paths, $messages);
        $report['brand_code_property_error_present'] = brandCodePropertyErrorPresent($paths, $messages);
        $report['string_type_error_present'] = stringTypeErrorPresent($paths, $messages);
    ...
    if ($bf7dOriginalFlagSnapshot !== null) {
        applyFlagSnapshot($bf7dOriginalFlagSnapshot);
    }
    $report['flags_after'] = flagSnapshot();
    $report['attempt_counts_after'] = attemptCountsSnapshot();
    $report['flags_restored'] = $bf7dOriginalFlagSnapshot !== null
        && flagsRestored($bf7dOriginalFlagSnapshot, $report['flags_after']);
}
