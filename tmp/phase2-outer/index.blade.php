{{--
  dashboard/customer/index.blade.php — Customer dashboard home (Phase 2 FULL REDESIGN).
  JETPK-DASHBOARD-UI-FOUNDATION · baseline 6fbfae4

  Replaces the previous stub (which ignored the controller data) with a populated home on
  the canonical customer-account portal shell — consistent with the bookings page and the
  Phase 1 shell, and at parity with the already-rich mobile dashboard.

  DATA CONTRACT (unchanged — from CustomerBookingController@dashboard, no new variables):
    $kpis['total'|'pending_payment'|'pnr_confirmed'|'cancellation_activity'] (int)
    $recentBookings (Collection<Booking>, ≤5)   $hasPendingPaymentBooking (bool)
    $firstPendingPaymentBooking (Booking|null)   $upcomingBooking (Booking|null)
    $upcomingCount (int)                          $supportTicketsCount (int)

  Uses the portal ota-account-* vocabulary + <x-dashboard.status-badge> (portal-styled)
  + ota-customer-dashboard.css. Route names and the customer-account layout are preserved.
  Requires the Phase 1 customer-account shell to be integrated (foundation assets).
--}}
