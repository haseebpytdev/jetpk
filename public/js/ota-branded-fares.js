...
(function (global) {
    ...
    function brandedFareOfferCheckedBaggage(offer) {
        if (!offer) {
            return '';
        }
        var direct = brandedFareFieldValue(offer, ['baggage_checked_display', 'check_in_summary', 'baggage_summary_display']);
        if (direct) {
            return direct;
        }
        var baggage = offer.baggage;
        if (baggage && typeof baggage === 'object') {
            if (baggage.checked) {
                return String(baggage.checked).trim();
            }
            if (baggage.summary) {
                return String(baggage.summary).trim();
            }
        }
        return '';
    }

    function brandedFareOfferCarryOnBaggage(offer) {
        if (!offer) {
            return '';
        }
        var direct = brandedFareFieldValue(offer, ['baggage_cabin_display', 'carry_on_summary']);
        if (direct) {
            return direct;
        }
        var baggage = offer.baggage;
        if (baggage && typeof baggage === 'object' && baggage.cabin) {
            return String(baggage.cabin).trim();
        }
        return '';
    }
    ...
    function brandedFareBenefitRows(opt, offer) {
        var resolved = {
            carry_on: '',
            check_in: '',
            meal: '',
        ...
        if (!resolved.carry_on) {
            setRow('carry_on', brandedFareOfferCarryOnBaggage(offer), 'offer carry-on');
        }
        if (!resolved.check_in) {
            setRow('check_in', brandedFareOfferCheckedBaggage(offer), 'offer checked');
        }
        if (!resolved.refund && offer && offer.refundable === false) {
            setRow('refund', 'Non-refundable', 'offer refundable');
        }
        ...
        var refundRaw = brandedFareFieldValue(opt, ['refundable_display', 'refund_rule', 'refund']) ||
            ...
            hints.refund ||
            '';
        var refundSummary = normalizeRefundSummaryForCard(refundRaw, opt.refundable != null ? opt.refundable : (offer ? offer.refundable : null));
        if (refundSummary === NOT_SPECIFIED_LABEL) {
    ...
    function brandedFareDedupeKey(opt) {
        ...
        var ctx = (opt && opt.provider_context && typeof opt.provider_context === 'object') ? opt.provider_context : {};
        var parts = [
            normalizeFareLabel(ctx.offer_ref_id || ''),
            normalizeFareLabel(ctx.offer_item_ref_id || ''),
            normalizeFareLabel(opt.name || opt.brand_name || ''),
            normalizeFareLabel(opt.fare_basis || ctx.fare_basis || ''),
            normalizeFareLabel(opt.booking_class || ctx.rbd || ''),
            normalizeFareLabel(opt.check_in_summary || ''),
            normalizeFareLabel(formatBrandedFarePrice(opt) || String(opt.displayed_price || opt.price_total || '')),
        ];
        return parts.join('|');
    }

    function brandedFareTitleBlock(opt) {
        var name = esc(opt.name || opt.brand_name || '');
        var subtitle = opt.fare_variant_subtitle || opt.fare_product_disambiguator || opt.brand_variant_hint || '';
        return '<div class="ota-branded-fare-card__title-block">' +
            '<h5 class="ota-branded-fare-card__name">' + name + '</h5>' +
            (subtitle ? '<p class="ota-branded-fare-card__variant">' + esc(subtitle) + '</p>' : '') +
            '</div>';
    }
    ...
    function brandedFaresVisibleCount() {
    ...
    function brandedFaresCarouselMaxIndex(cardCount, visibleCount) {
        return Math.max(0, cardCount - visibleCount);
    }

    function brandedFaresCarouselStepSize(viewport, grid, cards) {
        if (!cards.length) {
            return 0;
        }
        var gap = parseFloat(window.getComputedStyle(grid || viewport).gap || '12') || 12;
        return cards[0].offsetWidth + gap;
    }

    function brandedFaresCarouselSyncIndex(carousel) {
        ...
        var step = brandedFaresCarouselStepSize(viewport, grid, cards);
        if (!step) {
            return;
        }
        var index = Math.round(viewport.scrollLeft / step);
        var maxIndex = brandedFaresCarouselMaxIndex(cards.length, brandedFaresVisibleCount());
        ...
        brandedFaresCarouselSyncNavState(carousel, index, maxIndex);
    }

    function brandedFaresCarouselSyncNavState(carousel, index, maxIndex) {
        var prev = carousel.querySelector('[data-branded-fares-prev]');
        var next = carousel.querySelector('[data-branded-fares-next]');
        if (prev) {
    ...
    function brandedFaresSyncCarouselNav(panel) {
        ...
        var visibleCount = brandedFaresVisibleCount();
        var hideNav = renderedCount <= visibleCount || viewport.scrollWidth <= viewport.clientWidth + 2;
        ...
        if (!hideNav) {
            brandedFaresCarouselSyncIndex(carousel);
        }
    }
    ...
    function brandedFaresCarouselStep(carousel, direction) {
        ...
        var visibleCount = brandedFaresVisibleCount();
        var maxIndex = brandedFaresCarouselMaxIndex(cards.length, visibleCount);
        ...
        brandedFaresCarouselSyncNavState(carousel, nextIndex, maxIndex);
    }
    ...
    function buildPanelHtml(offer, state) {
        ...
        var cards = renderedFareOptions.map(function (opt) {
            ...
            var features = brandedFareBenefitRows(opt, offer);
    ...
    global.OtaBrandedFares = {
        ...
        brandedFareTitleBlock: brandedFareTitleBlock,
