...
| AD-007 | Temporary OTP-off via `OTA_CLIENT_REQUIRE_LOGIN_OTP=false` | Authorized QA window only; must restore before final PASS | Hidden auth route | Security exposure if not restored | Auth config | **Closed** — OTP required `true`; authorized `OTP_DEMO_*` retained from pre-cleanup backup |
...
| AD-010 | Legacy Admin/Staff Blade shells redirect to Next after parity | Retire presentation only; keep controllers/services; **no active Blade operator shell** | Delete Laravel admin routes | Broken bookmarks | Legacy retirement | **Adopted** — exhaustive redirect matrix + production crawl PASS |
| AD-011 | Inter as platform typeface via shared theme tokens | Brand consistency across public + portals + dashboard | Space Grotesk primary | Visual inconsistency | Design system | **Adopted** — production computed-style proof PASS |
| AD-012 | DB branding logo via existing public config pipeline | Avoid hard-coded text logos when upload exists | Static SVG only | Wrong brand in prod | Public + dashboard | **Adopted** — production logo probes PASS |
...
| AD-015 | Settings local-preview chrome is fixture/preview-only | Live production must not show "Local preview editing" residue | Keep preview editors on live | Crawl/acceptance FAIL | Settings UX | **Adopted (reopen)** |

## Open items (blocking ENGINEERING_ACCEPTANCE)

None remaining for engineering presentation/parity/legacy gates. Final SEC cleanup (QA deactivate + auth denial) is the last operational closure step and does not invent weaker architecture.
