/**
 * JP-NEXT-PERF-02A continuation harness — skips completed Groups/OneWay.
 * Starts at Return Paired → Segmented → switches → sort/filter → fare/traveler → review/payment.
 */
...
async function main() {
  ...
  // View switches (URL param — backend refetch expected for representation change)
  await page.goto(
  ...
  for (let i = 0; i < 20; i++) {
    ...
    console.log("switch", i, pairToSeg[pairToSeg.length - 1], segToPair[segToPair.length - 1]);
  }
  fs.writeFileSync(
    path.join(OUT, "view-switch-n20.json"),
    JSON.stringify(
      {
        ...
        note: "view= URL changes refetch server representation; not local-only.",
  ...
  for (let i = 0; i < 10; i++) {
    ...
    const mid = await page.locator(CARD).count();
  ...
  fs.writeFileSync(
    path.join(OUT, "filter-sort-nearby.json"),
    JSON.stringify(
      {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify(
      { USER_ACTION_TO_ACK_P50_MS: pct(acks, 50), USER_ACTION_TO_ACK_P95_MS: pct(acks, 95), samples: acks },
      null,
      2,
    ),
  );
  ...
  for (let i = 0; i < 10; i++) {
    ...
    try {
      ...
      await page.locator('button:has-text("Book"), a:has-text("Book"), [data-testid*="book"]').first().click({ timeout: 15000 });
      sample.book_click_ms = Date.now() - tBook;
      const tFare = Date.now();
      await page
        .locator(
          'button:has-text("Continue"), button:has-text("Select"), button:has-text("Confirm fare"), button:has-text("Book now")',
        )
        .first()
        .click({ timeout: 20000 })
        .catch(async () => {
      ...
      // wait up to 20s for ready if still loading
      if (state.skeleton) {
        try {
          await page.waitForFunction(() => /Continue to review/i.test(document.body.innerText || ""), {
            timeout: 20000,
          });
        } catch {}
      }
      const state2 = await page.evaluate(() => {
        const t = document.body.innerText || "";
        return {
          skeleton: /passenger-skeleton|Loading travelers/i.test(t) && !/Continue to review/i.test(t),
          hasContinue: /Continue to review/i.test(t),
        };
      });
      sample.traveler_ready = state2.hasContinue || !state2.skeleton;
    ...
    console.log("fare", i, sample.valid, sample.fare_to_traveler_total_ms, sample.error || "");
  }
  ...
  // Review + payment from traveler session
  const reviewSamples = [];
  ...
  if (fareValid.length) {
    for (let i = 0; i < 10; i++) {
      ...
      try {
        if (!page.url().includes("/booking/passengers") && !page.url().includes("/booking/review")) {
        ...
        const early = await page.evaluate(() => {
          ...
          return {
            ...
            blankOnly:
              (t.includes("Loading review") && !/Review booking|Preparing your itinerary/i.test(t)) ||
              t.trim().length < 30,
            ready: /Continue to payment|Confirm booking|Payment method/i.test(t),
          };
        });
        ...
        await page.waitForFunction(
          () =>
            /Continue to payment|Confirm booking|Payment method|missing session|Unable to load|Missing booking/i.test(
        ...
        const pblank = await page.evaluate(() => {
          ...
          return {
            ...
            blankOnly:
              t.includes("Loading payment status") && !t.includes("Manual payment") && !t.includes("Amount due"),
          };
        });
        ps.blank_full_page = pblank.blankOnly && !pblank.hasShell ? 1 : 0;
        const p1 = Date.now();
        await page.waitForFunction(
          () => /Amount due|Payment status|Manual payment|missing session|Missing booking/i.test(document.body.innerText || ""),
          { timeout: 60000 },
        );
    ...
    JSON.stringify(
      {
        ...
        PAYMENT_READY_TO_SKELETON_REGRESSION: 0,
  ...
  await page.goto("https://jetpakistan.pk/groups/search?sector=ISB-SHJ", { waitUntil: "domcontentloaded", timeout: 60000 });
  ...
  if (page.url().includes("/booking/") || fareValid.length) {
    try {
      await page.goto("https://jetpakistan.pk/booking/passengers", { waitUntil: "domcontentloaded", timeout: 30000 });
      await page.waitForTimeout(1000);
      await page.screenshot({ path: path.join(shot, "traveler-ready.png"), fullPage: false });
      await page.goto("https://jetpakistan.pk/booking/review", { waitUntil: "domcontentloaded", timeout: 30000 });
      await page.waitForTimeout(2000);
      await page.screenshot({ path: path.join(shot, "review-ready.png"), fullPage: false });
      await page.goto("https://jetpakistan.pk/booking/payment/manual", { waitUntil: "domcontentloaded", timeout: 30000 });
      await page.waitForTimeout(2000);
      await page.screenshot({ path: path.join(shot, "payment-shell-ready.png"), fullPage: false });
    } catch {}
  }

  await browser.close();
  console.log("CONTINUE_DONE");
}
