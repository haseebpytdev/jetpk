/**
 * JP-APP-PERF-CLOSURE-01R2 — Return Book Now → Traveler (reconciled timeline).
...
async function oneSample(browser, attempt) {
  const sample = {
    sample_id: `return-fare-01r2-${String(attempt).padStart(2, "0")}`,
    attempt,
    valid: false,
    mutation_posts: [],
  };
  const context = await browser.newContext({
    ...
    userAgent:
      "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/126.0.0.0 Safari/537.36 JP-APP-PERF-CLOSURE-01R2",
  });
...
async function main() {
  ...
  fs.writeFileSync(path.join(OUT, "traveler-warm-01r2-n30.json"), JSON.stringify(out, null, 2));
  console.log(
    JSON.stringify(
      {
        valid: valid.length,
