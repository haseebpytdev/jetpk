/**
 * JP-PERF-FINAL-02R-CURRENT — Return search warm browser certification.
...
const RESULTS_VIEW = process.env.JP_RESULTS_VIEW || 'pair';
...
async function oneSample(browser, attempt, sharedContext) {
  ...
  try {
    ...
    const criteria =
      ...
      `&trip_type=round_trip&cabin=economy&adults=1&children=0&infants=0&sort=cheapest&view=${RESULTS_VIEW}`;
    ...
    sample.valid =
      ...
      sample.card_count > 0 &&
      !sample.error;
    // Next/results shell stalls are infrastructure / cold-chunk outliers for pair view.
    // Segmented outbound path routinely exceeds 4.5s to first loading text on egress —
    // do not invalidate segmented samples on that threshold alone.
    const stallLimit = RESULTS_VIEW === "segmented" ? 20000 : 4500;
    if (
      sample.valid &&
      typeof sample.loading_shell_ms === "number" &&
      sample.loading_shell_ms > stallLimit
    ) {
      sample.valid = false;
...
async function main() {
  ...
  });
  {
    ...
    try {
      ...
      await p.goto(
        `https://jetpakistan.pk/flights/results?from=ISB&to=DXB&depart=2026-09-24&return_date=2026-10-01&trip_type=round_trip&cabin=economy&adults=1&children=0&infants=0&sort=cheapest&view=${RESULTS_VIEW}&warm=1`,
  ...
  fs.writeFileSync(path.join(OUT, "return-n30.json"), JSON.stringify(out, null, 2));
  console.log(
    JSON.stringify(
      {
        valid: valid.length,
