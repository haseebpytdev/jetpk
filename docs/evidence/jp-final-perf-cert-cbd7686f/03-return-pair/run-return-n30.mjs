...
async function main() {
  ...
  while (samples.filter((s) => s.valid).length < TARGET && attempt < MAX_ATTEMPTS) {
    ...
    console.log(
      JSON.stringify({
        ...
        post_supplier: s.BROWSER_RENDER_MS,
  ...
  const out = {
    ...
    RETURN_POST_SUPPLIER_TO_USABLE_P50_MS: pct(pick("BROWSER_RENDER_MS"), 50),
    RETURN_POST_SUPPLIER_TO_USABLE_P95_MS: pct(pick("BROWSER_RENDER_MS"), 95),
    ...
    JSON.stringify(
      {
        ...
        post_supplier_p50: out.RETURN_POST_SUPPLIER_TO_USABLE_P50_MS,
        post_supplier_p95: out.RETURN_POST_SUPPLIER_TO_USABLE_P95_MS,
        pair_to_browser_p95: out.PAIR_AVAILABLE_TO_BROWSER_P95_MS,
        poll_total_p95_diagnostic: out.POLL_TOTAL_SERVER_P95_MS,
