...
| `/__dev/jetpk-homepage-v2` (rewrite → `/dev/jetpk-homepage-v2`) | `frontend/app/dev/jetpk-homepage-v2/page.tsx` | `noindex,nofollow` | Phase D static homepage composition review; gated by `isThemeLabAllowed()`; not in navigation or sitemap; **not counted** in production route target |
