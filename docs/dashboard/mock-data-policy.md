...
- Loaded via [`dashboard/services/overview-service.ts`](../../dashboard/services/overview-service.ts) and [`dashboard/services/booking-service.ts`](../../dashboard/services/booking-service.ts)

## Recoverable error preview (bookings)

Append `previewError=1` to the bookings query string to simulate a recoverable service error (deterministic, for QA only).
