...
| Preview UI | `http://localhost:3001/testdash` | Mock-only Next shell + overview + bookings + payments + customers + suppliers |
...
5. **Future API seam** — [`dashboard/services/overview-service.ts`](../../dashboard/services/overview-service.ts), [`dashboard/services/booking-service.ts`](../../dashboard/services/booking-service.ts), [`dashboard/services/payment-service.ts`](../../dashboard/services/payment-service.ts), [`dashboard/services/customer-service.ts`](../../dashboard/services/customer-service.ts), and [`dashboard/services/supplier-service.ts`](../../dashboard/services/supplier-service.ts) swap mock for Laravel JSON later.
...
```text
dashboard/
  app/           Route segments (overview, bookings, payments, customers, suppliers, planned stubs)
  components/    ui + dashboard chrome
  features/      overview + bookings + payments + customers + suppliers modules
  layouts/       DashboardShell
  lib/           preview, nav, bookings/payments/customers/suppliers query/filter, utils
