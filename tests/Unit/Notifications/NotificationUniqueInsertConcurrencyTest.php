...
class NotificationUniqueInsertConcurrencyTest extends TestCase
{
    public function test_two_php_processes_same_event_id_yield_one_row(): void
    {
        $dir = sys_get_temp_dir().DIRECTORY_SEPARATOR.'jp-notif-'.bin2hex(random_bytes(4));
        mkdir($dir, 0777, true);
        ...
        $table = 'notification_outbox_race';

        $pdo = new \PDO('sqlite:'.$db);
        $pdo->setAttribute(\PDO::ATTR_ERRMODE, \PDO::ERRMODE_EXCEPTION);
        $pdo->exec('PRAGMA journal_mode=WAL');
        $pdo->exec('CREATE TABLE '.$table.' (event_id TEXT PRIMARY KEY)');

        $actor = base_path('tests/bin/notification-unique-insert-actor.php');
