...
class AlHaiderClientManualTokenConnectionTest extends TestCase
{
    ...
    public function test_managed_token_used_even_when_limit_blocked(): void
    {
        Cache::put('alhaider:auth_token:limit_blocked', true, 300);
        ...
        SupplierConnection::query()->create([
            ...
            'credentials' => [
                ...
                'existing_token' => 'managed-live-token',
                'token_expires_at' => '2027-08-11T10:13:18Z',
                'auto_renew' => '0',
            ],
        ]);
        ...
        Http::fake([
            'alhaider.test/api/available/groups*' => function ($request) {
                $this->assertSame('Bearer managed-live-token', $request->header('Authorization')[0] ?? '');

                return Http::response(['groups' => [['id' => 1]]], 200);
            },
        ]);

        $probe = app(AlHaiderClient::class)->probeAuthentication();
        $this->assertSame('ok', $probe['reason_code']);
        $this->assertTrue($probe['token_obtained']);
    ...
    public function test_missing_token_produces_controlled_error(): void
    {
        $agency = Agency::factory()->create();
        SupplierConnection::query()->create([
            'agency_id' => $agency->id,
