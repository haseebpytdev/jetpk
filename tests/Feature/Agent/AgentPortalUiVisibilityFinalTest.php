...
/**
 * Final UI visibility UAT for agent portal nav, CTAs, and permission-gated elements.
 */
class AgentPortalUiVisibilityFinalTest extends TestCase
{
    ...
    public function test_agent_admin_sees_full_finance_and_operations_ctas(): void
    {
        ...
        $this->actingAs($admin)->get(route('agent.dashboard'))
            ...
            ->assertSee('data-testid="account-dropdown-desktop"', false);
    }
    ...
    public function test_header_dropdown_hides_restricted_links_for_staff_a0(): void
    {
        $scenario = $this->buildAgentPortalScenario();
        $staff = $scenario['staff']['A0'];
        ...
        $this->actingAs($staff)->get(route('agent.dashboard'))
            ...
            ->assertDontSee('>Ledger<', false)
            ->assertDontSee('>Deposits<', false)
            ->assertDontSee('>Commissions<', false);
    }
}
