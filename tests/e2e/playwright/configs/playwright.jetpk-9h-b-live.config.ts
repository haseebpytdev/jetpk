...
export default defineConfig({
  testDir: path.join(process.cwd(), 'tests/playwright/jetpk-9h-b'),
  outputDir: path.join(process.cwd(), 'tests/playwright/artifacts/jetpk-9h-b/live-results'),
  globalSetup: path.join(process.cwd(), 'tests/playwright/jetpk-9h-b/global-setup-live.ts'),
  timeout: 180_000,
  fullyParallel: false,
  workers: 1,
  reporter: [
