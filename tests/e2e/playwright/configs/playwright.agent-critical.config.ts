/**
 * Fast agent + agent staff critical responsive checks (local Chromium only).
 */
...
export default defineConfig({
  ...
  testMatch: /agent-critical-responsive\.spec\.ts/,
  timeout: 60_000,
  ...
  retries: 0,
  reporter: [['list']],
  outputDir: path.join(process.cwd(), 'UI_test/traces/critical'),
  use: {
  ...
  globalSetup: path.join(process.cwd(), 'tests/visual/global-setup.ts'),
  projects: [
    {
      name: 'setup',
