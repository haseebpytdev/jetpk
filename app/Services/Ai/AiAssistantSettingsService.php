    ...
    public function update(User $actor, array $payload): AiAssistantSetting
    {
        ...
        if (array_key_exists('audience_mode', $payload)) {
            ...
            if ($mode === AiAssistantSetting::AUDIENCE_PUBLIC) {
                if (! ($hard['public'] ?? false)) {
                    throw new \InvalidArgumentException('Public rollout is blocked by environment hard ceiling.');
                }
            } elseif (! in_array($mode, [AiAssistantSetting::AUDIENCE_OFF, AiAssistantSetting::AUDIENCE_INTERNAL_CANARY], true)) {
