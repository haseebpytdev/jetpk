...
/**
 * Read-only orchestration for Ameer-e-Millat Umrah group package search and detail.
 */
class AmeerEMillatGroupService
{
    private const CACHE_PREFIX = 'ameer_e_millat:umrah_groups:';

    private const STALE_SUFFIX = ':stale';

    private const AIRLINES_CACHE_KEY = 'ameer_e_millat:airlines';

    public function __construct(
        private readonly AmeerEMillatClient $client,
        private readonly AmeerEMillatPackageNormalizer $normalizer,
    ) {}
    ...
    public function search(array $filters = [], bool $forceFresh = false): UmrahGroupSearchResultData
    {
        ...
        if (! $this->client->isConfigured()) {
            return new UmrahGroupSearchResultData(
                api_disabled: true,
                warnings: ['Ameer-e-Millat credentials are not configured.'],
        ...
        try {
            ...
            $ttl = max(60, (int) config('suppliers.ameer_e_millat.cache_ttl_seconds', 600));
            ...
            $reason = $exception instanceof AmeerEMillatProviderException ? $exception->errorCode : 'exception';

            Log::warning('ameer_e_millat.search_failed', [
    ...
    public function getPackageDetail(string $publicId, bool $forceFresh = false): ?UmrahGroupPackageData
    {
        ...
        if (! (bool) config('suppliers.ameer_e_millat.enabled') || ! $this->client->isConfigured()) {
            return null;
        }
        ...
        try {
            ...
            Log::warning('ameer_e_millat.detail_failed', [
                'supplier' => 'ameer_e_millat',
    ...
    private function resolveAirlineMap(): array
    {
        ...
        Cache::put(self::AIRLINES_CACHE_KEY, $map, max(300, (int) config('suppliers.ameer_e_millat.cache_ttl_seconds', 600)));
    ...
    private function packageFromArray(array $row): UmrahGroupPackageData
    {
        return new UmrahGroupPackageData(
            supplier: (string) ($row['supplier'] ?? 'ameer_e_millat'),
    ...
    private function resolveGroupId(string $publicId): string
    {
        ...
        if (str_starts_with(strtoupper($publicId), 'AEM-')) {
