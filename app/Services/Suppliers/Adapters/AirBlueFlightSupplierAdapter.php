...
class AirBlueFlightSupplierAdapter implements FlightSupplierInterface
{
    public function __construct(
        private readonly AirBlueFlightSearchService $searchService,
        private readonly AirBlueOfferPriceService $offerPriceService,
    ...
    public function search(FlightSearchRequestData $request, SupplierConnection $connection): FlightSearchResultData
    {
        if (! $this->connectionReady($connection)) {
            $this->diagnosticLogger->log(
                ...
                action: 'search',
                status: 'warning',
                safeMessage: 'Connection is inactive for AirBlue search.',
            );

            return new FlightSearchResultData(
                supplier_provider: SupplierProvider::Airblue,
                offers: [],
                warnings: ['AirBlue supplier connection is inactive.'],
    ...
    public function validateOffer(
        ...
        if (is_string($offer)) {
            return new OfferValidationResultData(
                ...
                warnings: ['AirBlue offer snapshot is required for validation.'],
            );
        }
        ...
        return $this->offerPriceService->revalidate($offer, $connection);
    }

    public function provider(): SupplierProvider
    {
        return SupplierProvider::Airblue;
    }
