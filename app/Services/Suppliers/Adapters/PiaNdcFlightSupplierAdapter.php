...
class PiaNdcFlightSupplierAdapter implements FlightSupplierInterface
{
    public function __construct(
        private readonly PiaNdcFlightSearchService $searchService,
    ...
    public function search(FlightSearchRequestData $request, SupplierConnection $connection): FlightSearchResultData
    {
        if (! $this->connectionReady($connection)) {
            $this->diagnosticLogger->log(
                ...
                action: 'search',
                status: 'warning',
                safeMessage: 'Connection is inactive for PIA NDC search.',
            );

            return new FlightSearchResultData(
                supplier_provider: SupplierProvider::PiaNdc,
                offers: [],
                warnings: ['PIA NDC supplier connection is inactive.'],
    ...
    public function validateOffer(
        ...
        if (is_string($offer)) {
            return new OfferValidationResultData(
                ...
                warnings: ['PIA NDC offer snapshot is required for validation.'],
            );
        }
