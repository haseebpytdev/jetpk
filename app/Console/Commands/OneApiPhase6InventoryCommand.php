...
/**
 * Generates Phase 6 isolation package manifests and v6 backup/rollback/deploy scripts (no git staging).
 */
class OneApiPhase6InventoryCommand extends Command
{
    protected $signature = 'ota:one-api-phase-6-inventory';

    protected $description = 'Generate phase-6 One API runtime manifests, v6 backup/rollback/deploy artifacts.';

    public function handle(): int
    {
        $runtimePath = storage_path('app/one-api-phase-6-runtime-files.txt');
        ...
        $this->write(storage_path('app/one-api-deploy-files-v6.txt'), $deploy);
        $this->writeSftpV6($deploy);
        $this->writeBackupV6($deploy);
        $this->writeRollbackV6($deploy);
        $this->writePostDeployV6();
        $this->writeRequiredConfigV6();
     ...
     */
    private function writeSftpV6(array $deploy): void
    {
        ...
        $this->write(storage_path('app/one-api-sftp-upload-v6.txt'), $lines);
    }

    /**
     * @param  list<string>  $deploy
     */
    private function writeBackupV6(array $deploy): void
    {
        ...
        $script = [
            ...
            'set -eu',
            'APP_ROOT="'.$appRoot.'"',
            'BACKUP_DIR="/home/pkjetp/backups/one-api-v6-$(date +%Y%m%d%H%M%S)"',
        ...
        file_put_contents(storage_path('app/one-api-predeploy-backup-v6.sh'), implode(PHP_EOL, $script).PHP_EOL);
    }

    /**
     * @param  list<string>  $deploy
     */
    private function writeRollbackV6(array $deploy): void
    {
        $appRoot = '/home/pkjetp/jetpk_app';
        ...
        $script = [
            ...
            '  echo "Usage: $0 /home/pkjetp/backups/one-api-v6-YYYYMMDDHHMMSS" >&2',
            '  exit 1',
            'fi',
            'MANIFEST="$BACKUP_DIR/manifest.tsv"',
            'if [ ! -f "$MANIFEST" ]; then',
        ...
        file_put_contents(storage_path('app/one-api-rollback-v6.sh'), implode(PHP_EOL, $script).PHP_EOL);
    }

    private function writePostDeployV6(): void
    {
        $php = '/opt/alt/php-fpm83/usr/bin/php';
        file_put_contents(storage_path('app/one-api-post-deploy-v6.sh'), implode(PHP_EOL, [
            '#!/bin/sh',
            'set -eu',
            'cd /home/pkjetp/jetpk_app',
    ...
    private function writeRequiredConfigV6(): void
    {
        file_put_contents(storage_path('app/one-api-required-config-v6.md'), implode(PHP_EOL, [
            '# One API required configuration (v6)',
            '',
    ...
    private function writeExcludedRuntime(): void
    {
        ...
        $this->write(storage_path('app/one-api-phase-6-excluded-files.txt'), $excluded);
    }
    ...
    private function writeIsolationPackage(): void
    {
        ...
        $docs = [];
        $generated = [];
        $runtime = $this->readLines(storage_path('app/one-api-phase-6-runtime-files.txt'));
        foreach ($all as $path) {
        ...
        $this->write(storage_path('app/one-api-phase-6-tests.txt'), $tests);
        $this->write(storage_path('app/one-api-phase-6-docs.txt'), $docs);
        $this->write(storage_path('app/one-api-phase-6-shared-files.txt'), $shared);
        $this->write(storage_path('app/one-api-phase-6-mixed-files.txt'), $mixed);
        $this->write(storage_path('app/one-api-phase-6-public-mirrors.txt'), ['public/js/ota-one-api-checkout.js => /home/pkjetp/public_html/js/ota-one-api-checkout.js']);
        ...
        $this->write(storage_path('app/one-api-phase-6-runtime-files.txt'), $runtime);

        $stageNew = ['# Review-only staging (do not run automatically)'];
        foreach ($new as $file) {
            $stageNew[] = 'git add -- '.$file;
        }
        file_put_contents(storage_path('app/one-api-phase-6-stage-new-files.ps1'), implode(PHP_EOL, $stageNew).PHP_EOL);
        ...
        file_put_contents(storage_path('app/one-api-phase-6-stage-shared-files.ps1'), implode(PHP_EOL, $stageShared).PHP_EOL);
        ...
        file_put_contents(storage_path('app/one-api-phase-6-interactive-stage.md'), $md);
        ...
        file_put_contents(storage_path('app/one-api-phase-6-review.patch'), ($patch !== '' ? $patch : "# No indexed diff for runtime slice; use git diff on working tree before commit.\n").PHP_EOL);
    }
}
