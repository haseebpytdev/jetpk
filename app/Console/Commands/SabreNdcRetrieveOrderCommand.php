...
use App\Services\Suppliers\Sabre\Ndc\SabreNdcOrderRetrieveService;
use App\Support\Sabre\SabreMutationCommandGate;
use Illuminate\Console\Command;

class SabreNdcRetrieveOrderCommand extends Command
{
    protected $signature = 'sabre:ndc-retrieve-order
                            {--booking= : Booking ID}
                            {--dry-run : Preview only (default)}
                            {--send : Live retrieve HTTP}
                            {--confirm= : RETRIEVE-NDC-ORDER-FOR-BOOKING-{id}}';

    protected $description = 'Sabre NDC order retrieve — orders/view + ndc/orders/retrieve fallback';

    public function handle(SabreNdcOrderRetrieveService $service): int
    {
        ...
        $expected = 'RETRIEVE-NDC-ORDER-FOR-BOOKING-'.$booking->id;
        $gate = SabreMutationCommandGate::evaluate(
            (bool) $this->option('dry-run'),
            $this->option('send') !== null ? '1' : null,
            $this->option('confirm'),
            $expected,
            ['suppliers.sabre.ndc.enabled'],
        );

        $connection = $this->resolveConnection($booking);
        $result = $service->retrieveOrder($booking, $connection, ! $gate['live_allowed']);
        $result['gate'] = $gate;
