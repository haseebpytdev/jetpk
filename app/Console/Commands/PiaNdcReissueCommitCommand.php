...
class PiaNdcReissueCommitCommand extends Command
{
    protected $signature = 'pia-ndc:reissue-commit
        {booking : OTA booking ID}
        {--connection= : Supplier connection ID}';

    protected $description = 'Run PIA NDC DoReissueCommit for an OTA booking';
    ...
    public function handle(PiaNdcReissueService $reissueService): int
    {
        ...
        try {
            $result = $reissueService->commit($booking, $connection);
            $this->line(json_encode($result, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES));

            return self::SUCCESS;
        } catch (PiaNdcProviderException $exception) {
            $this->error('reissue_commit_failed='.$exception->safeMessage);
