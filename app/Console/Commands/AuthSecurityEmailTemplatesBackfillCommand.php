...
class AuthSecurityEmailTemplatesBackfillCommand extends Command
{
    protected $signature = 'ota:backfill-auth-email-templates
                            {--dry-run : Preview rows to create or update without writing}
                            {--force : Overwrite existing template subject/body with defaults}
                            {--agency= : Limit to one agency ID}';

    protected $description = 'Backfill auth/security email templates into agency_message_templates (K2D-A)';
    ...
    public function handle(AgencyMessageTemplateSeeder $seeder): int
    {
        ...
        if ($dryRun) {
            ...
            $this->warn('Force mode — existing auth/security template copy will be overwritten.');
        } else {
            $this->info('Creating missing auth/security template rows only (existing rows are preserved).');
        }
        ...
        $query->chunkById(50, function ($agencies) use ($dryRun, $force, $seeder, &$stats): void {
            foreach ($agencies as $agency) {
                ...
                $agencyStats = $seeder->seedAuthSecurityDefaultsForAgency($agency, $force, $dryRun);
