...
class IatiTestBookingCommand extends Command
{
    protected $signature = 'iati:test-booking
        {booking : OTA booking ID}
        {--connection= : Supplier connection ID}
        {--dry-run : Build payloads only without live call}';

    protected $description = 'Execute or dry-run IATI booking for an OTA booking';

    public function handle(IatiBookingService $bookingService): int
    {
    ...
    protected function resolveConnection(Booking $booking): ?SupplierConnection
    {
        ...
        return $cid > 0
            ? SupplierConnection::query()->find($cid)
