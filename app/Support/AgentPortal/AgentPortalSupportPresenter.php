...
/**
 * Agent support ticket JSON for Next.js dashboard.
 */
class AgentPortalSupportPresenter
{
    /**
    ...
    public function presentListItem(SupportTicket $ticket): array
    {
        return [
            ...
            'detail_url' => '/agent/support/'.$ticket->ticket_reference,
        ...
        return [
            ...
            'conversation' => $ticket->messages
                ->map(fn ($message) => [
                    ...
                    'author_role' => $message->user_id === $ticket->created_by_user_id ? 'agent' : 'staff',
                    'body' => $message->body,
                    'created_at' => $message->created_at?->toIso8601String(),
                ])
                ->values()
                ->all(),
            'reply_url' => '/laravel/agent/support/tickets/'.$ticket->ticket_reference.'/reply',
        ];
    }
