...
use App\Policies\CustomerQueryPolicy;
...
class AppServiceProvider extends ServiceProvider
{
    ...
    public function boot(): void
    {
        ...
        RateLimiter::for('abhipay-payment-callback', fn (Request $request): Limit => Limit::perMinute(60)->by($request->ip()));
        ...
        Gate::policy(CustomerQuery::class, CustomerQueryPolicy::class);
        ...
        View::composer(
            ...
            function ($view): void {
                if (str_starts_with($view->name(), 'themes.frontend.jetpakistan.')) {
                    $view->with([
                ...
                if ($view->name() === 'errors.layout' && function_exists('request_client_slug_for_errors') && request_client_slug_for_errors() === 'jetpk') {
                    return;
                }
        ...
        View::composer('dashboard.admin.*', function ($view): void {
            if (! is_client_preview()) {
                return;
            }
            $profile = app(\App\Services\Client\CurrentClientContext::class)->get();
            if ($profile === null || trim((string) $profile->active_admin_theme) !== 'jetpakistan') {
                return;
            }
            $view->with('jpLegacyAdminShell', true);
        });

        View::composer('themes.frontend.jetpakistan.layouts.frontend', function ($view): void {
            if (! is_client_preview()) {
                return;
            }
            $paletteCss = app(\App\Services\Branding\ClientThemePaletteService::class)->cssVariablesForProfile();
            if ($paletteCss !== []) {
                $view->with('jpApprovedPaletteCss', $paletteCss);
            }
        });
    }
}
