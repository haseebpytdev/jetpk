...
const expectedCount = Number(process.env.JP_UI_05_EXPECTED_COUNT ?? "132");

if (!existsSync(manifestPath)) {
  console.error(`[verify-jp-ui-05] Missing manifest: ${manifestPath}`);
  process.exit(1);
}
...
function fail(message) {
  console.error(`[verify-jp-ui-05] ${message}`);
  exitCode = 1;
}
...
if (exitCode === 0) {
  console.log(`[verify-jp-ui-05] PASS expected=${expectedCount} actual=${captures.length} passed=${manifest.passed ?? captures.length} manifest=${manifestPath}`);
}
