...
const CASE_SEQUENCE = [
  ...
  { kind: "multi", id: "08-positive-confirmation", steps: ["LHE to DXB tomorrow", "one way", "1 adult", "yes confirm"], assert: (t, _s, steps) => assertPattern(t, /LHE|DXB|search|confirm|flight/i, steps), extra: { confirmation_state: "positive" } },
  { kind: "multi", id: "09-negative-confirmation", steps: ["LHE to DXB tomorrow one way 1 adult", "no"], assert: (t, _s, steps) => assertPattern(t, /not|cancel|change|help|LHE|DXB/i, steps), extra: { confirmation_state: "negative" } },
  { kind: "multi", id: "10-correction-after-recap", steps: ["LHE to DXB tomorrow one way 1 adult", "actually make it DXB to LHE"], assert: (t, _s, steps) => assertPattern(t, /DXB|LHE|correct|change|confirm/i, steps), extra: { confirmation_state: "corrected" } },
  ...
  { kind: "handoff_decline", id: "20-handoff-decline", steps: ["connect me to support", "no thanks continue with AI"], assert: (t, _s, steps) => { assertAssistantSubstantive(t, steps); }, extra: { handoff_state: "declined" } },
  ...
  { kind: "independent", id: "24-rag-conflict", inputs: ["is carry-on free on all airlines"], assert: (t, _s, inputs) => assertPattern(t, /airline|baggage|fare|class|depends|allowance/i, inputs), extra: { rag_behavior: "conflict" } },
