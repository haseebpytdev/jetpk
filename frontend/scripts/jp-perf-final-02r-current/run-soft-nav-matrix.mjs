...
const EVIDENCE_DIR = process.env.JP_PERF_OUT_DIR || path.resolve(__dirname, "../../../docs/evidence/jp-perf-final-02r-current");
...
async function softClickNav(page, href, opts = {}) {
  ...
  const clicked = await page.evaluate(async (targetHref) => {
    ...
    let el = findLink();
    if (!el) {
      // Open one dropdown at a time — opening every menu mounts all Links and
      // stampses Next RSC prefetch (lookup-booking + siblings) into the click path.
      const triggers = Array.from(document.querySelectorAll('nav button[aria-haspopup="menu"]'));
...
const out = {
  ...
  APPLICATION_CONTROLLED_MULTI_SECOND_ROUTE_COUNT: matrix.filter((m) => (m.APPLICATION_CONTROLLED_P95 || 0) > 1500).length,
...
fs.writeFileSync(path.join(EVIDENCE_DIR, "soft-nav-matrix.json"), JSON.stringify(out, null, 2));
console.log(
  JSON.stringify(
    {
