...
].map(([name, price, baggage, refund, change], index) => ({ option_key: `fare-${index + 1}`, name, displayed_price: price, price_display: `${price} PKR`, baggage, refund_rule: refund, change_rule: change, meal: index > 0 ? "Meal included" : undefined, seat_selection: index > 1 ? "Seat included" : "Paid selection", selection_key_authoritative: true, can_select: true }));

function offer(index: number) {
  const direct = index !== 1;
  return { offer_id: `offer-${index + 1}`, airline_code: ["PK", "EK", "QR", "SV"][index], airline_name: ["Pakistan International Airlines", "Emirates", "Qatar Airways", "Saudia"][index], departure_time: ["07:10", "09:35", "13:20", "20:45"][index], arrival_time: ["10:25", "14:40", "16:35", "23:55"][index], duration: direct ? "3h 15m" : "5h 05m", stops: direct ? 0 : 1, displayed_price: 78812 + index * 8110, final_customer_price: 78812 + index * 8110, can_book: true, refundable: index > 1, baggage: "30kg checked", flight_number: `${["PK", "EK", "QR", "SV"][index]}${301 + index}`, segments: [{ origin_airport_code: "ISB", destination_airport_code: direct ? "DXB" : "DOH", departure_time_display: ["07:10", "09:35", "13:20", "20:45"][index], arrival_time_display: direct ? ["10:25", "14:40", "16:35", "23:55"][index] : "11:30", flight_number: `${["PK", "EK", "QR", "SV"][index]}${301 + index}` }, ...(direct ? [] : [{ origin_airport_code: "DOH", destination_airport_code: "DXB", departure_time_display: "12:35", arrival_time_display: "14:40", flight_number: "EK401" }])], layover_summary_display: direct ? [] : ["1h 05m layover · DOH"], branded_fares_display_options: fares, fare_family_options_display: fares, has_branded_fares: true };
}

function details(selected = fares[0]) {
  const base = Number(selected.displayed_price) - 18312;
  return { success: true, search_id: searchId, offer_id: "offer-1", offer: { ...offer(0), displayed_price: selected.displayed_price, final_customer_price: selected.displayed_price, branded_fares_display_options: fares, fare_family_options_display: fares, baggage_summary_display: selected.baggage, refund_rule: selected.refund_rule, change_rule: selected.change_rule, fallback_details: { baggage: { summary: selected.baggage, cabin: "1 cabin bag up to 7kg", checked: selected.baggage, passenger_baggage: [{ passenger_type: "ADULT", cabin: "7kg", checked: selected.baggage }] }, fare_rules: { refund_rule: selected.refund_rule, change_rule: selected.change_rule, penalty: "Supplier penalty applies before departure", rule_lines: ["No-show rule supplied by airline"] }, fare_breakdown: { base_fare: base, taxes: 18312, grand_total: selected.displayed_price, displayed_price: selected.displayed_price, currency: "PKR", passenger_pricing: [{ passenger_type: "adult", passenger_count: 1, base_amount: base, tax_amount: 18312, total_amount: selected.displayed_price, currency: "PKR" }] } }, select_url: "/booking/passengers" } };
}
