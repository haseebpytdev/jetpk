...
export const AUDIT_ROOT = path.resolve(process.cwd(), ".visual-audit", "jp-ui-04");
...
export function writeManifest(): void {
    ...
    JSON.stringify(
      {
        ...
        expectedScenarioCount: Number(process.env.JP_UI_04_EXPECTED_COUNT ?? "0"),
  ...
  writeFileSync(
    SUMMARY_PATH,
    JSON.stringify({ generatedAt: new Date().toISOString(), captureCount: captureRecords.length, passed, failed, manifestPath: MANIFEST_PATH }, null, 2),
    "utf8",
  );
}
...
export async function applyTheme(page: Page, theme: ThemeMode) {
  ...
  return { preference, colorScheme: emulateDark ? "dark" : "light" as const };
}
