...
import { JP_UI_06_WAVE_1_SCENARIOS, WAVE_1_SCENARIO_COUNT } from "./jp-ui-06-scenarios";

const SERVER_PORT = Number(process.env.JP_UI_06_PORT ?? process.env.PLAYWRIGHT_PORT ?? "3002");
const EXPECTED_RUN_COUNT = Number(process.env.JP_UI_06_EXPECTED_COUNT ?? String(WAVE_1_SCENARIO_COUNT));

test.describe.configure({ mode: "serial" });

test.describe("JP-UI-06 Wave 1 blueprint captures", () => {
  test.beforeAll(() => {
    mkdirSync(AUDIT_ROOT, { recursive: true });
  });

  for (const scenario of JP_UI_06_WAVE_1_SCENARIOS) {
...
const WAVE_1_OVERFLOW_PROBES = [
  { name: "320x700", width: 320, height: 700 },
  { name: "375x812", width: 375, height: 812 },
  { name: "390x844", width: 390, height: 844 },
  { name: "768x1024", width: 768, height: 1024 },
  { name: "1024x900", width: 1024, height: 900 },
  { name: "1122x1330", width: 1122, height: 1330 },
];

test.describe("JP-UI-06 Wave 1 overflow probes", () => {
  for (const scenario of JP_UI_06_WAVE_1_SCENARIOS.filter((s) => s.state === "canonical-light-desktop")) {
    for (const probe of WAVE_1_OVERFLOW_PROBES) {
    ...
    test(`overflow-${scenario.family}-zoom-150`, async ({ page }) => {
      await setupJpUi06Scenario(page, scenario);
      await applyTheme(page, "light");
      const route = resolveJpUi06Route(scenario);
