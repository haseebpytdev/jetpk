...
import { setupPublicBaseline } from "./jp-ui-03a-fixtures";
import {
  AUDIT_ROOT,
  applyTheme,
  applyZoom,
  ...
  captureScenario,
  writeManifest,
} from "./jp-ui-03a-helpers";
import { EXPECTED_SCENARIO_COUNT, JP_UI_03A_SCENARIOS } from "./jp-ui-03a-scenarios";
...
test.afterAll(() => {
  writeManifest();
  const expectedCount = Number(process.env.JP_UI_03A_EXPECTED_COUNT ?? EXPECTED_SCENARIO_COUNT);
...
for (const scenario of JP_UI_03A_SCENARIOS) {
  test(`jp-ui-03a ${scenario.id} ${scenario.theme} @ ${scenario.viewport.name} [${scenario.state}]`, async ({ page }) => {
    if (scenario.setup) {
