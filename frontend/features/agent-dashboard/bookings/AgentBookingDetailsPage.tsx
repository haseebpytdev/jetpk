...
export function AgentBookingDetailsPage({ session, reference }: { session: PublicSession; reference: string }) {
  ...
  const load = async () => {
    setLoading(true);
    setError(null);
    const result = await fetchAgentBookingDetail(reference);
    if (!result.ok) {
  ...
  return (
    <AgentDashboardShell session={session} title={`Booking ${reference}`}>
      ...
      {data?.ok ? (
        <div data-testid="agent-booking-detail">
