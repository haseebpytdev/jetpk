...
@section('content')
    @php
        ...
        $pendingDepositsUrl = client_route('admin.agent-deposits.index', ['status' => 'submitted']);
        ...
        <section class="mb-4" aria-labelledby="ota-action-queue-heading">
            ...
            <div class="ota-dash-action-grid" data-testid="ota-action-queue">
                @foreach ($actionCards as $card)
                    @php
                        $href = client_route($card['route'], $card['route_params'] ?? []);
            ...
            <div class="ota-dash-shortcuts" data-testid="ota-admin-quick-actions">
                @foreach ($shortcuts as $shortcut)
                    <x-dashboard.overview.shortcut-chip
                        :href="client_route($shortcut['route'], $shortcut['route_params'] ?? [])"
                ...
                <section class="card ota-dash-panel h-100" data-testid="ota-dash-recent-activity">
                    <div class="card-header py-2 border-0">
                        <div class="d-flex justify-content-between align-items-center gap-2">
                            ...
                            <a href="{{ client_route('admin.bookings') }}" class="small text-decoration-none">View all activity</a>
                        </div>
                    </div>
                    <div class="card-body pt-0">
                        @forelse ($activityItems as $item)
                            @php
                                $itemHref = client_route($item['route'] ?? 'admin.bookings', (array) ($item['route_params'] ?? []));
                            @endphp
            ...
            <div class="col-lg-5">
                <section class="card ota-dash-panel h-100" data-testid="ota-dash-system-status">
                    ...
                    <div class="card-body pt-0">
                        ...
                        <div class="pt-2">
                            <a href="{{ client_route('admin.api-settings') }}" class="small text-decoration-none">View system logs <i class="ti ti-chevron-right"></i></a>
                        </div>
                    </div>
                </section>
            </div>
