...
@endphp
<section id="ota-flight-search" class="ota-search-widget-section {{ $isFloating ? 'ota-search-widget-section--floating' : '' }}" data-airport-widget="{{ $widgetId }}" data-min-date="{{ $minDate }}" data-trip-type="{{ $defaultTripType }}" data-airports-search-url="{{ url('/airports/search') }}" data-layout="{{ $layout }}">
    <div class="ota-search-card {{ $variant === 'standalone' ? 'ota-search-card--standalone' : '' }} {{ $isFloating ? 'ota-search-card--floating ota-search-card--horizontal' : '' }}">
        ...
        <div class="ota-search-card-body {{ $isFloating ? 'ota-search-card-body--floating' : '' }}">
            ...
            <form method="get" action="{{ client_route('flights.results') }}" class="ota-flight-form {{ $isFloating ? 'ota-flight-form--floating ota-flight-form--horizontal' : '' }}" id="{{ $widgetId }}-form" data-flight-search-form novalidate>
            ...
            <div data-trip-panel="one_way" style="{{ $defaultTripType !== 'one_way' && $defaultTripType !== 'round_trip' ? 'display:none;' : '' }}">
                ...
                <div class="ota-from-to-row {{ $isFloating ? 'ota-search-hbar__route' : '' }}">
                    <div class="ota-from-wrap ota-hbar-cell ota-hbar-cell--from">
                    ...
                    <div class="ota-to-wrap ota-hbar-cell ota-hbar-cell--to">
                ...
                <div class="ota-search-dates ota-hbar-cell ota-hbar-cell--dates {{ $defaultTripType === 'round_trip' ? 'ota-search-dates--round' : '' }} {{ $isFloating ? 'ota-search-dates--floating' : '' }}" data-search-dates>
            ...
            <div class="ota-search-pax-grid">
                ...
                <div class="ota-select-shell">
                    ...
                    <div class="ota-select-shell__inner">
                        ...
                        <select class="ota-field ota-field--shell ota-field--select" id="{{ $widgetId }}-infants" name="infants" data-infants-select>
                            @php $widgetInfantMax = min(9, max(1, (int) old('adults', 1))); @endphp
                            @for ($i = 0; $i <= $widgetInfantMax; $i++)
            ...
            <div class="ota-flight-info-bar" role="note">
                ...
                <span>Adults min 1 · total passengers max 9 · one infant per adult</span>
            </div>
            @endunless
...
(function () {
  ...
  widgets.forEach(function (widget) {
    ...
    function syncInfantsSelect() {
      var adultsEl = widget.querySelector('[name="adults"]');
      var infantsEl = widget.querySelector('[name="infants"]');
    ...
    widget.querySelectorAll('[name="adults"], [name="children"], [name="infants"], [name="cabin"]').forEach(function (el) {
