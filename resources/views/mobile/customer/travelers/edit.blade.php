...
@section('content')
    <div class="ota-mobile-customer" data-testid="ota-mobile-customer-travelers-edit">
        @if ($errors->any())
            @include('mobile.components.alert', ['type' => 'danger', 'message' => $errors->first()])
        @endif

        <form method="post" action="{{ route('customer.travelers.update', $traveler) }}" class="ota-mobile-customer__form" data-testid="saved-traveler-form">
            @csrf
            @method('PUT')
