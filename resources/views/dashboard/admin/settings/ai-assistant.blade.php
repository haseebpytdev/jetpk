    ...
    <div class="mx-auto max-w-4xl space-y-6 p-6">
        ...
        <form method="post" action="{{ route('admin.settings.ai-assistant.update') }}" class="space-y-6">
            ...
            <section class="rounded-xl border border-slate-200 bg-white p-5">
                ...
                @php($hardPublic = (bool) config('ota.ai_assistant.hard_allow.public', false))
                @if ($hardPublic)
                    <div class="mt-4 flex flex-col gap-2 border-t border-slate-100 pt-4 sm:flex-row sm:items-center sm:justify-between">
                        <div>
                            <div class="font-medium text-slate-900">Public audience</div>
                            <div class="mt-1 text-xs text-slate-500">Requires environment public hard allow. Guests see Ask when effective mode is public.</div>
                        </div>
                        <div class="form-check form-switch">
                            <input
                                class="form-check-input"
                                ...
                                value="1"
                                id="public_enabled"
                                @checked(old('public_enabled', ($status['mode'] ?? '') === 'public'))
                            >
                            <label class="form-check-label text-sm text-slate-700" for="public_enabled">Enable public</label>
                        </div>
                    </div>
                @endif
        ...
        <section class="rounded-xl border border-slate-200 bg-white p-5">
            ...
            <dl class="mt-4 grid gap-4 sm:grid-cols-2">
                ...
                <div>
                    ...
                    <dd class="mt-1 font-medium {{ ($status['mode'] ?? '') === 'public' ? 'text-emerald-700' : 'text-slate-700' }}">
                        {{ ($status['mode'] ?? '') === 'public' ? 'AUTHORIZED / ON' : (config('ota.ai_assistant.hard_allow.public') ? 'AUTHORIZED / OFF' : 'NOT AUTHORIZED') }}
                    </dd>
                </div>
            </dl>
        </section>
