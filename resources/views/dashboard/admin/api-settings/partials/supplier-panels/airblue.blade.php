<div class="jp-provider-panel {{ $isAirblue ? '' : 'jp-is-hidden' }}" data-provider-panel="airblue">
    <div class="jp-form-grid jp-form-grid--2">
        <div class="jp-field">
            <label class="form-label" for="airblue-channel">API channel</label>
            <select id="airblue-channel" name="credentials[api_channel]" class="form-select" data-airblue-channel @disabled(! $isAirblue)>
        ...
        <div class="jp-field jp-field--full">
            <label class="form-label" for="airblue-base-url">Endpoint URL</label>
            <input id="airblue-base-url" type="url" name="base_url" class="form-control" value="{{ old('base_url', $connection->base_url) }}" required data-airblue-base-url placeholder="https://app.crane.aero/cranendc/v20.1/CraneNDCService" @disabled(! $isAirblue)>
